...
function colorForProject(p: Project) {
  if (!p.confirmed) return { style: { backgroundColor: 'hsl(0 90% 96%)', color: 'hsl(0 60% 30%)', borderColor: 'hsl(0 70% 72%)' }, dashed: true }
...
function useIsPortrait() {
  ...
  useEffect(() => {
    ...
    onChange(); mq.addEventListener('change', onChange); window.addEventListener('resize', onChange)
...
function usePreview() {
  ...
  const open = useCallback((project: Project, x: number, y: number) => {
    ...
    setState({ project, x: Math.min(Math.max(m, x), vw - m), y: Math.min(Math.max(m, y), vh - m) })
  }, [])
...
function Popover({ state, onClose }: { state: PreviewState, onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    document.addEventListener('keydown', onKey); document.addEventListener('mousedown', onClick); document.addEventListener('touchstart', onClick)
  ...
  return (
    <div className="fixed inset-0 z-[60] pointer-events-none">
      ...
      <div ref={boxRef} className="absolute pointer-events-auto translate-x-[-50%] translate-y-[-50%] rounded-xl border shadow-lg bg-white p-3 w-[240px] max-w-[85vw]"
...
function MiniProjectCard({
  ...
  return (
    ...
    >
      <div ref={ref} className={`absolute top-1 left-1 pr-6 max-w-[80%] truncate pointer-events-none ${titleClass}`} title={nameText}>
        ...
        return (
          <div key={wIdx} className="grid grid-cols-7 gap-[4px] md:gap-1 p-[4px] bg-gray-100">
            {week.map(iso => {
              ...
              const entry = mapProjects.get(iso)!; const pDay = entry.list; const count = entry.count
...
/* ============== WeekView (portrait con swipe) ============== */
function WeekView({
  items, monthStart, monthEnd, calStart, calEnd, onPreview, weekIndex, onWeekIndexChange
}:{
  items: Project[]; monthStart: string; monthEnd: string; calStart: string; calEnd: string;
  onPreview:(p:Project,x:number,y:number)=>void; weekIndex:number; onWeekIndexChange:(i:number)=>void
}) {
  const days = useMemo(()=>eachDayISO(calStart, calEnd), [calStart, calEnd])
  const weeks = useMemo(()=>chunkWeeks(days), [days])

  // precompute proyectos/día
  const mapProjects = useMemo(()=>{
    const m = new Map<string, { list: Project[]; count: number }>()
    for (const iso of days) {
      const inRange = items.filter(p => dayInTotalRange(p, iso))
  ...
  useEffect(()=>{
    ...
    el.scrollTo({ left: weekIndex * el.clientWidth, behavior: 'instant' as any })
  },[weekIndex])

  const onScroll = useCallback(()=>{
    const el = scRef.current; if(!el) return
    const i = Math.round(el.scrollLeft / el.clientWidth)
    if(i!==weekIndex) onWeekIndexChange(Math.max(0, Math.min(weeks.length-1, i)))
  },[weekIndex, weeks.length, onWeekIndexChange])

  const goto = (delta:number)=>{
    const i = Math.max(0, Math.min(weeks.length-1, weekIndex+delta))
    onWeekIndexChange(i)
    const el = scRef.current; if(el) el.scrollTo({ left: i * el.clientWidth, behavior: 'smooth' })
  }
  ...
  return (
    <div className="rounded-2xl border shadow-sm overflow-hidden">
      ...
      >
        <div className="flex w-full">
          {weeks.map((week, wIdx)=>{
            ...
            const hClass = heightClassByCount(maxCount)
            return (
              <div key={wIdx} className="w-full shrink-0 snap-start p-[4px] bg-gray-100">
      ...
      <div className="flex items-center justify-center gap-2 py-2">
        <button onClick={()=>goto(-1)} className="px-2.5 py-1.5 rounded-lg border text-sm" disabled={weekIndex<=0}>←</button>
        <div className="text-xs text-gray-500">{weekIndex+1} / {weeks.length}</div>
        <button onClick={()=>goto(1)} className="px-2.5 py-1.5 rounded-lg border text-sm" disabled={weekIndex>=weeks.length-1}>→</button>
      </div>
    </div>
  )
}
...
export default function CalendarPage() {
  ...
  // índice de semana activo (para portrait)
  ...
  // recomponer layout en cambios de viewport
  useEffect(() => {
  ...
  // calcular semana del "hoy" o primera semana del mes al entrar en portrait
  useEffect(()=>{
    if(!isPortrait) return
    const days = eachDayISO(calStart, calEnd)
    const weeks = chunkWeeks(days)
    const today = todayISO()
    const idx = Math.max(0, weeks.findIndex(w => w[0] <= today && today <= w[w.length-1]))
    setWeekIndex(idx === -1 ? 0 : idx)
  },[isPortrait, calStart, calEnd])
  ...
  function gotoMonth(delta:number){
    const d=parseISO(monthStart); d.setUTCMonth(d.getUTCMonth()+delta,1); setMonthAnchor(startOfMonthISO(d))
  }

  const monthLabel = new Intl.DateTimeFormat('es-ES',{month:'long',year:'numeric',timeZone:'UTC'}).format(parseISO(monthStart))
    ...
    <div className="h-full flex flex-col relative select-none">
      ...
      <div className="p-3 md:p-6 space-y-3">
        ...
        <div className="flex items-center justify-center gap-2">
          ...
          <div className="text-base md:text-lg font-medium">{monthLabel}</div>
          <button onClick={()=>gotoMonth(1)} className="px-2.5 py-1.5 rounded-lg border text-sm">→</button>
        </div>
